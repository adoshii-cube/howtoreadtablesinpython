...
fp = open(filesname[0], 'rb')
# Use custom global function convert_pdf_to_text, to convert entire pdf to string and run bag of words over it
degreeCheck = fun_isGrad(convert_pdf_to_txt(filesname[0]).replace("\n", " "))
print "degreeCheck :::: ", degreeCheck
...
for pageNumber, page in enumerate(PDFPage.create_pages(doc)):
    ...
    if(found):
        for lt_obj in layout:
            try:
                if(lt_obj.get_text()):
                    perc=re.findall(r'[0-9\.]{0,3}\d{1,2}[\s\.]{0,1}', lt_obj.get_text())
                    if(len(perc)>0):
                        numbers.extend([perc[0]])
                        yValues.append(lt_obj.y0)
            except:
                continue
            print "pageNumber :::: ", pageNumber
            print "yValues (BEFORE) ::::: ", yValues
        ...
        try:
            ...
            pass
        print "yValues (AFTER) ::::: ", yValues
        break

if((found == True) & (len(yValues) > 0)):
    ind = yValues.index(min(map(abs, yValues)))
    marks = numbers[ind]
    print "marks ::: ", marks
